	...
	class d {
		...
		get CSS() { 
			return {
				...
				content2: this._data.service // ruizyi
			}
		}
		render() {
			...
			const {
				html: t
			} = d.services[this.data.service], i = document.createElement("div"), e = document.createElement("div"), s = document.createElement("template"), n = this.createPreloader();
			i.classList.add(this.CSS.baseClass, this.CSS.container, this.CSS.containerLoading), e.classList.add(this.CSS.input, this.CSS.caption), i.appendChild(n), e.contentEditable = !this.readOnly, e.dataset.placeholder = this.api.i18n.t("설명문 캡션"), e.innerHTML = this.data.caption || "", s.innerHTML = t, s.content.firstChild.setAttribute("src", this.data.embed), s.content.firstChild.classList.add(this.CSS.content), s.content.firstChild.classList.add(this.CSS.content2) ;
		...
		onPaste(t) {
			const {
				...
				height: a,
				id: l = f => f.shift()
			} = d.services[i], m = s.exec(e).slice(1), h = n.replace(/<%= remote_id %>/g, l(m));
			this.data = {
				service: i,
				source: e,
				embed: h,
		...
		static prepare({
			...
			const s = Object.entries(i).filter(([r, a]) => typeof a == "boolean" && a === !0).map(([r]) => r),
				n = Object.entries(i).filter(([r, a]) => typeof a == "object").filter(([r, a]) => d.checkServiceConfig(a)).map(([r, a]) => {
					const {
						regex: l,
						embedUrl: m,
						html: h,
			...
			s.length && (e = e.filter(([r]) => s.includes(r))), e = e.concat(n), d.services = e.reduce((r, [a, l]) => a in r ? (r[a] = Object.assign({}, r[a], l), r) : (r[a] = l, r), {}), d.patterns = e.reduce((r, [a, l]) => (r[a] = l.regex, r), {})
		}
		...
		embedIsReady(t) {
			...
			return new Promise((s, n) => {
				e = new MutationObserver(g.debounce(s, 450)), e.observe(t, {
					childList: !0,
					subtree: !0
				})
			}).then(() => {
